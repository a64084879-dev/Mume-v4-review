...
COMMISSION = US_FEE
B1_PCTL = 0.75
EXIT_INDEX = "GSPC"; GATE_MODE = "ABS"; REC_HOT_INDEX = "GSPC"   # ★R9c: 탈출선 = S&P500(복귀선 공통)
...
def _signals(d):
    ...
    sig = {
        # ★R9c: 탈출 기준선 = EXIT_INDEX 스위치 분기(NDX면 현행 그대로 — 기본값 수학적 동일 보장)
        "G":    sh(d["NDX"] if EXIT_INDEX == "NDX" else d["GSPC"]),
        "GS":   sh(d["NSMA"] if EXIT_INDEX == "NDX" else d["GSMA"]),
