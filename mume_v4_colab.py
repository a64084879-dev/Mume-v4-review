...
# ★A(2026-07-24): 복귀 안내를 정확히 하려면 자동복귀 여부도 알아야 한다.
#   AUTO_MODE=on인데 AUTO_RECOVER=off면 러너가 복귀를 집행하지 않는데(수동 /enter 정책)
#   그 /enter는 러너 가드가 거부한다 → CASH 고착. 리포트가 "이미 접수"라고 하면 오보다.
AUTO_RECOVER_HINT = str(os.environ.get("AUTO_RECOVER", "off")).strip().lower() == "on"
...
def apply_command(pos, text, price_hint, Veff_target=None):
    ...
    def _px(default):
        for tok in t:
            if tok.startswith("@"):
                try: _v=float(tok[1:].replace(",",""))
                except Exception:
                    raise ValueError(f"가격 '{tok}' 파싱 실패 — 숫자를 확인하세요 (예: @75.5). 조용한 종가 대체를 막기 위해 거부")
                # ★입력검증 패치(2026-07-27): 0·음수·nan·inf 가격 차단. @0/@음수는 '공짜 주식·현금 증발'
                #   원장을 만들고, nan은 모든 크기 가드를 False로 통과해 원장을 오염시킨다(배터리B10 실측).
                if not np.isfinite(_v) or _v<=0:
                    raise ValueError(f"가격 '{tok}' 거부 — 0보다 큰 유한한 숫자만 됩니다 (예: @75.5)")
                return _v
        return default
    def _num(tok, what="주수"):
        # ★입력검증 패치(2026-07-27): 주수·금액 공용 가드. nan/inf만 차단.
        #   부호·0 허용 여부는 각 명령의 기존 가드가 그대로 판정한다(예: /lumpsum_done 0, 인출 음수).
        _v=float(tok)
        if not np.isfinite(_v):
            raise ValueError(f"{what} '{tok}' 거부 — nan/inf는 원장을 오염시킵니다. 유한한 숫자만 입력하세요")
        return _v
    try:
        if cmd=="/buy":
            ...
            sh=_num(t[1]); px=_px(price_hint); cost=sh*px
        ...
        if cmd=="/enter":
            ...
            amt=_num(t[1],"금액") if len(t)>1 and not t[1].startswith("@") else _default
            if amt<=0:
            ...
            pos["cyc_used"]=0.0
            pos.pop("ladder_placed_for",None); pos.pop("rsv_ladder_for",None)   # 복귀 직후 새 사다리 자동 재게시(N1) + ★W7 예약 래치 해제
        ...
        if cmd=="/setpos":
            ...
            try:
                # ★입력검증 패치(2026-07-27): 셋 다 파싱 성공한 뒤 일괄 대입.
                #   (종전: 두번째 값이 불량이면 shares만 먼저 바뀐 채 거부 — 부분기록. nan도 차단)
                _sh=_num(t[1],"주수"); _pl=_num(t[2],"현금"); _vv=_num(t[3],"V")
            except Exception:
                return pos,"⚠️ 주수·현금·V는 유한한 숫자로 넣으세요. 예) /setpos 86 1661.69 0 2026-07-05 INVESTED"
            pos["shares"]=_sh; pos["pool"]=_pl; pos["V"]=_vv
            pos["last_cycle_start"]=dstr; pos["state"]=st_in
            pos["last_recover_check"]=dstr
            pos["pending_deposit"]=0.0; pos["pending_lump"]=0.0
            for _k in ("pending_lump_mode","evac_sig_date","ladder_placed_for","pending_echo","lump_in_flight",
        ...
        if cmd=="/lumpsum":
            ...
            if not np.isfinite(amt):   # ★입력검증 패치(2026-07-27)
                return pos,"⚠️ 금액 거부 — nan/inf는 원장을 오염시킵니다. 유한한 숫자만 입력하세요."
            if amt==0: return pos,"⚠️ 금액이 0입니다."
            _mode=(t[2].lower() if len(t)>2 else ""); _prev_lump=float(pos.get("pending_lump",0.0) or 0.0)
            _force=(_mode=="pool!")   # ★P2(2026-07-31 은박사 승인): 임계 초과 의도적 강행 표기
            if _force: _mode="pool"
            ...
            if _mode=="pool":
                # ★P2(2026-07-31 은박사 승인): pool 임계 = 총자산의 2%. 큰 금액의 pool 오용이
                #   V 왜곡(왕복 시 영구 잔존, 자동 복구 경로 없음)을 만드는 허점 차단(은박사 발견).
                #   초과는 거부+안내. 자동 v 전환은 하지 않는다(지시 없는 주문 금지). 강행='pool!'만.
                #   정당한 pool 용도(분기배당≈총자산 0.15%·이자·정산차이)는 전부 임계 미만 — 오탐 0.
                if (not _force) and total>0 and abs(amt)>total*0.02:
                    return pos,(f"⚠️ pool 모드는 총자산의 2%까지만 허용합니다 (요청 {abs(amt):,.0f} / 한도 {total*0.02:,.0f}).\n"
                                f"   pool = 배당·이자·정산차이 등 <b>이미 계좌에 반영된 현금</b>의 인식 전용입니다.\n"
                                f"   · 새 자금이면 → <code>/lumpsum {amt:+.0f} v</code> (비중대로 자동 배분, V도 함께 조정)\n"
                                f"   · 그래도 pool로 처리하려면 → <code>/lumpsum {amt:+.0f} pool!</code>")
                _newpool=pool+amt
                if _newpool<0:
        ...
        if cmd=="/lumpsum_done":
            ...
            if not np.isfinite(sh):   # ★입력검증 패치(2026-07-27)
                return pos,"⚠️ 주수 거부 — nan/inf는 원장을 오염시킵니다. 유한한 숫자만 입력하세요."
            px=_px(price_hint)
...
def compute_ladder(shares, V, pool, max_rows=None, sell_reach=1.50, budget_override=None, cur_px=None):
    ...
    for _ in range(8):                        # 안전 한계(무한루프 방지)
        ...
        if _n <= HARD: break
        # ★정정(2026-07-24): 이 식은 이름과 달리 실질 +1 스텝이다. _count_buy가 HARD+1에서
        #   캡되므로 _n/HARD ≤ 61/60 → 배율이 1을 겨우 넘어 max(buy_lot+1, ...)이 지배한다.
        #   다만 초기 buy_lot이 est 기반이라 이산오차가 ≤+2 수준이고, 실측(12만주×pool 9종)에서
        #   절단 0건·2회 내 수렴을 확인했으므로 그대로 둔다. 서술만 바로잡는다.
...
def compute_signal(df, pos):
    ...
    if state=="INVESTED" and action_ks is None:
        ...
        if ev < bmin - 1e-6:
            ...
            band_msg.append("   → 러너가 이미 사다리를 걸어두었고 체결은 자동 동기화됩니다." if AUTO_MODE_HINT
                            else "   → 체결됐으면 /buy 로 보고, 사다리를 안 걸었으면 /ladder 로 받아 걸어두세요.")
        elif ev > bmax + 1e-6:
            band_msg.append("🟥 평가금이 상한 위 — 정상이면 사다리 매도가 이미 체결됐을 구간입니다.")
            band_msg.append("   → 러너가 이미 사다리를 걸어두었고 체결은 자동 동기화됩니다." if AUTO_MODE_HINT
                            else "   → 체결됐으면 /sell 로 보고, 사다리를 안 걸었으면 /ladder 로 받아 걸어두세요.")
        else:
    ...
    return dict(today=today,dec_date=str(today.date()),p=p,pr=pr,bub=bub,pc=pc,g=g,gs=gs,ndx=ndx,nsm=nsm,rv=rv,scale=scale,
                ...
                # ★표시 전용(2026-07-24): day_only(매일 재배치)에서 헤더를 실체에 맞게 쓰기 위한 값.
                #   판정·계산에는 일절 쓰이지 않는다.
                rem_budget=(max(0.0, float(pos.get("cyc_budget",0.0))-float(pos.get("cyc_used",0.0)))
                            if pos.get("cyc_budget") is not None else None),
                cyc_day=(wall_today.date()-cyc_start).days+1)

# ══════════════ [5. 리포트] ══════════════
def render_echo_head(pos, cmd_results):
    """★R-A(2026-07-24): pending_echo의 '지난 실행 처리분'을 리포트 머리말로 렌더.
       봇 main과 러너가 같은 로직을 쓰도록 헬퍼로 분리(러너에 미러 누락 → 재전송 안전망 무효였음).
       cmd_results가 있으면 echo의 마지막 원소가 이번 실행분이므로 그 앞만 ⏮로 낸다."""
    head=[]
    _echo = pos.get("pending_echo") or []
...
def clear_echo(pos, save=True):
    """★R-A: 발송 성공 시에만 호출. 실패면 잔존시켜 다음 실행에 ⏮로 재전송한다."""
    if pos.get("pending_echo"):
        pos["pending_echo"]=[]
        if save: save_position(pos)
    return pos
...
def build_report(s, df):
    ...
    L.append(f"\n<b>[시장]</b>")
    L.append(f"TQQQ(실제가) {s['pr']:,.2f} | 버블(S&P500÷M0) {s['bub']:.2f}"
    ...
    if s['state']=="INVESTED" and s.get('action_ks') is None:
        if s.get('ladder_lot')==-1:
            ...
            if AUTO_MODE_HINT:
                # ★day_only 실체 반영(2026-07-24): 러너가 매일 취소·재배치하므로 "사이클 시작일에
                #   한 번 걸고 2주 유지"는 사실이 아니다. 체결·잔여한도 변화로 그리드도 매일 움직인다.
                _rb=s.get("rem_budget"); _rbs=f" · 잔여한도 ${_rb:,.0f}" if _rb is not None else ""
                L.append(f"\n<b>[사다리 지정가 — 오늘 재배치분]</b> 사이클 {s.get('cyc_day',1)}일차{_rbs}")
                L.append(f"  러너가 기존 미체결을 전량 취소하고 아래로 새로 걸었습니다(매일 재배치).")
            else:
            ...
            if s.get('sell_ladder'):
                ...
                for pt,sh,pl in s['sell_ladder']:
                    L.append(f"     @ {pt:,.2f} → {sh}주 보유")
            L.append("  ※ 러너가 매일 재배치합니다(체결·잔여한도 반영). 체결은 자동 동기화 — 보고 불필요." if AUTO_MODE_HINT
    ...
    if s.get('copy_cmd'):
        ...
        if AUTO_MODE_HINT:
            ...
            _is_rec = str(s.get('copy_cmd') or '').strip().lower().startswith('/enter')
            L.append(f"⏰ <b>실행 (자동)</b>")
            L.append(f"   이 신호는 <b>{s['dec_date']} 종가({s['pr']:,.2f})로 판정</b>한 것입니다.")
            if _is_rec and not AUTO_RECOVER_HINT:
                # ★A(2026-07-24): 복귀인데 자동복귀가 꺼짐 → 러너가 접수하지 않는다.
                #   그런데 수동 /enter도 러너 가드가 거부하므로 CASH에 갇힌다. 정확히 알린다.
                L.append(f"   🚨 <b>자동복귀가 꺼져 있어(AUTO_RECOVER=off) 러너가 접수하지 않았습니다.</b>")
                L.append(f"   ⚠️ AUTO_MODE에선 <code>/enter</code>도 거부되어 <b>CASH에 고착</b>됩니다.")
                L.append(f"   → 조치: 환경변수 <code>AUTO_RECOVER=on</code>으로 바꾼 뒤 다시 실행하세요.")
                L.append(f"     (즉시 필요하면 크론 정지 → 수동봇으로 <code>/enter</code> → 재개)")
            else:
                L.append(f"   → 러너가 <b>이번 실행에서 이미 접수</b>했습니다(장중 marketable limit).")
                L.append(f"   ※ 체결 확정은 다음 실행의 API 동기화로 반영 — <b>별도 보고 불필요</b>합니다.")
...
def main():
    ...
    head=render_echo_head(pos, cmd_results)   # ★R-A: 러너와 공유하는 헬퍼
    if cmd_results:
    ...
    if sent_ok:
        _ping()
        clear_echo(pos)                    # ★S4/R-A: 발송 성공 시에만 소거(헬퍼 공유)
