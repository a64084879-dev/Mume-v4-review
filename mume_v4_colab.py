...
REAL_STARTS = ["2010-02-11", "2013-01-02", "2016-01-02",
               "2019-01-02", "2022-01-02", "2024-01-02"]
END_DATE    = "2026-07-10"

# ★★ v2 핵심 스위치 ★★
SIGNAL_LAG = 1

# 진단만 빠르게 확인하기 위해 기존 표 및 차트 비활성화
RUN_HOLD, RUN_DCA, RUN_WD = "off", "off", "off"
KILLSWITCH = "on"
CHART_ON   = "off"
CHART_MODE = "hold"
...
def build_data_fallback(db=""):
    """M0를 '앵커 보간'으로 대체한 데이터셋.
       실 FRED(m0_full.csv) 대신, 봇이 FRED 실패 시 쓰는 폴백 경로를 재현한다.
       무매에서 이 차이로 버블 1.37→1.34, B1 성과가 완전히 뒤집힌 바 있다."""
    ndx, irx, gspc, qqq_real, tqqq_real, _m0_real = get_sources(db)
    # ★폴백 M0 (십억$) — 역사적 앵커 보간
    ...
    m0 = (s.reindex(pd.date_range("1985-01-01", "2026-12-31", freq="YS"))
    ...
    if tqqq_real_al is not None:
        ...
        ret_for_rv = ret_syn
    out["RV"] = ret_for_rv.rolling(VOLTGT_LOOKBACK).std() * np.sqrt(252)
    print(f"  · [폴백 M0] 버블 최신 {out['BUB'].iloc[-1]:.3f} "
          f"(실 FRED와 비교하십시오)")
    return out
...
# ══════════════ [8. 진단 A — VOLTGT 2×2 (same-bar 무해성)] ══════════════
def voltgt_lag_2x2(df, init=None, pool_ratio=None, G=None, buy_limit=None):
    global SIGNAL_LAG, VOLTGT_ON
    init = init or HOLD_CAP
    pool_ratio = HOLD_POOL if pool_ratio is None else pool_ratio
    G = G or HOLD_G; buy_limit = buy_limit or HOLD_LIMIT
    sv_lag, sv_vt = SIGNAL_LAG, VOLTGT_ON
    print("\n" + "=" * 108)
    print("  🔬 [진단 A] VOLTGT 2×2 — same-bar 집행 덕이었나")
    print("     기여 = (VOLTGT on) − (VOLTGT off), 같은 LAG 안에서 비교")
    print("     LAG0 기여 >> LAG1 기여  →  VOLTGT 효과는 실전 불가능한 이득이었음")
    print("=" * 108)
    print(f"{'시작일':<13}{'LAG0 기여ΔCAGR':>15}{'LAG1 기여ΔCAGR':>15}"
          f"{'LAG0 기여ΔMDD':>15}{'LAG1 기여ΔMDD':>15}{'판정':>12}")
    print("-" * 108)
    for sd in REAL_STARTS:
        sub = df[df.index >= sd]
        if END_DATE: sub = sub[sub.index <= END_DATE]
        ...
        for lag in (0, 1):
            for vt in ("off", "on"):
                SIGNAL_LAG = lag; VOLTGT_ON = vt
                r = run_vr(sub, init, pool_ratio, G, buy_limit, killswitch=ON(KILLSWITCH))
                res[(lag, vt)] = (r["cagr"], r["mdd"])

        c0 = (res[(0, "on")][0] - res[(0, "off")][0]) * 100
        c1 = (res[(1, "on")][0] - res[(1, "off")][0]) * 100
        m0 = (res[(0, "on")][1] - res[(0, "off")][1]) * 100
        m1 = (res[(1, "on")][1] - res[(1, "off")][1]) * 100

        if abs(c0) > 0.3 and (c1 / c0 if c0 != 0 else 1) < 0.5:
            verdict = "⚠ same-bar"
        else:
            verdict = "✅ 무해"
        print(f"{sd:<13}{c0:>+14.2f}%{c1:>+14.2f}%{m0:>+14.2f}%{m1:>+14.2f}%{verdict:>12}")
    SIGNAL_LAG, VOLTGT_ON = sv_lag, sv_vt
    print("=" * 108)
    print("  · 기여ΔMDD +면 VOLTGT가 낙폭 개선. LAG1(봇정합) 기여가 실제로 얻는 값이다.")
    print("  · LAG0에서만 크고 LAG1에서 사라지면 → VOLTGT 채택 근거 무효.")
    print("=" * 108)


# ══════════════ [9. 진단 B — 오버레이 조합 (v2 기준, 실데이터)] ══════════════
def combo_table_v2(df, init=None, pool_ratio=None, G=None, buy_limit=None):
    global SIGNAL_LAG, VOLTGT_ON, B1_ON, KILLSWITCH
    init = init or HOLD_CAP
    pool_ratio = HOLD_POOL if pool_ratio is None else pool_ratio
    G = G or HOLD_G; buy_limit = buy_limit or HOLD_LIMIT
    sv = (SIGNAL_LAG, VOLTGT_ON, B1_ON, KILLSWITCH)
    SIGNAL_LAG = 1
    combos = [("순수 VR",      "off", "off", "off"),
              ("VOLTGT만",     "on",  "off", "off"),
              ("B1만",         "off", "on",  "on"),
              ("VOLTGT+B1",    "on",  "on",  "on")]
    print("\n" + "=" * 112)
    print("  🔬 [진단 B] 오버레이 조합 — SIGNAL_LAG=1(봇 정합) · 실데이터 기준")
    print("=" * 112)
    for sd in REAL_STARTS:
        sub = df[df.index >= sd]
        if END_DATE: sub = sub[sub.index <= END_DATE]
        if len(sub) < 300: continue
        print(f"\n▸ 시작일 {sd}")
        print(f"{'조합':<14}{'세후NAV':>16}{'CAGR':>9}{'MDD':>9}{'샤프':>8}"
              f"{'ΔCAGR':>9}{'ΔMDD':>9}{'Δ샤프':>8}{'대피':>7}")
        print("-" * 96)
        base = None
        for name, vt, b1, ks in combos:
            VOLTGT_ON, B1_ON, KILLSWITCH = vt, b1, ks
            r = run_vr(sub, init, pool_ratio, G, buy_limit, killswitch=ON(ks))
            if base is None:
                base = (r["cagr"], r["mdd"], r["sharpe"])
                dc = dm = ds = 0.0
            else:
                dc = (r["cagr"] - base[0]) * 100
                dm = (r["mdd"] - base[1]) * 100
                ds = r["sharpe"] - base[2]
            print(f"{name:<14}{r['aftertax']:>16,.0f}{r['cagr']*100:>8.1f}%"
                  f"{r['mdd']*100:>8.1f}%{r['sharpe']:>8.2f}"
                  f"{dc:>+8.1f}%{dm:>+8.1f}%{ds:>+8.2f}{r['n_exit']:>7}")
    SIGNAL_LAG, VOLTGT_ON, B1_ON, KILLSWITCH = sv
    print("\n" + "=" * 112)
    print("  · Δ = 순수 VR 대비. ΔMDD +면 낙폭 개선. 시너지 = (둘다 Δ) > (VOLTGT Δ + B1 Δ)")
    print("  · 총배수 비율 = 조합NAV / 순수NAV. 1 초과여야 순이익.")
    print("=" * 112)


# ══════════════ [10. 진단 B-0 — B1 임계가 살아있는가] ══════════════
def b1_threshold_autopsy(df, init=None, pool_ratio=None, G=None, buy_limit=None):
    global B1_ON, B1_PCTL, VOLTGT_ON, KILLSWITCH, SIGNAL_LAG
    init = init or HOLD_CAP
    pool_ratio = HOLD_POOL if pool_ratio is None else pool_ratio
    G = G or HOLD_G; buy_limit = buy_limit or HOLD_LIMIT
    sv = (B1_ON, B1_PCTL, VOLTGT_ON, KILLSWITCH, SIGNAL_LAG)
    SIGNAL_LAG = 1
    VOLTGT_ON = "off"
    KILLSWITCH = "on"
    arms = [("KS only(B1 off)", "off", None),
            ("B1 PCTL=0.70",    "on",  0.70),
            ("B1 PCTL=0.75",    "on",  0.75),
            ("B1 PCTL=0.80",    "on",  0.80),
            ("B1 PCTL=0.85",    "on",  0.85),
            ("B1 PCTL=0.90",    "on",  0.90)]
    print("\n" + "=" * 104)
    print("  🔬 [진단 B-0] B1 임계 부검 — 버블 백분위는 실제로 일을 하는가")
    print("     PCTL=0.70 ~ PCTL=0.90 사이에서 대피 횟수가 전부 같다면 → 백분위는 장식, B1 = SMA200 추세추종")
    print("     (VOLTGT off · SIGNAL_LAG=1 · 킬스위치 on)")
    print("=" * 104)
    for sd in REAL_STARTS:
        sub = df[df.index >= sd]
        if END_DATE: sub = sub[sub.index <= END_DATE]
        if len(sub) < 300: continue
        print(f"\n▸ 시작일 {sd}")
        print(f"{'조건':<18}{'세후NAV':>15}{'CAGR':>8}{'MDD':>9}{'샤프':>7}{'대피':>6}{'복귀':>6}")
        print("-" * 76)
        for name, b1, pctl in arms:
            B1_ON = b1
            if pctl is not None: B1_PCTL = pctl
            r = run_vr(sub, init, pool_ratio, G, buy_limit, killswitch=True)
            print(f"{name:<18}{r['aftertax']:>15,.0f}{r['cagr']*100:>7.1f}%"
                  f"{r['mdd']*100:>8.1f}%{r['sharpe']:>7.2f}{r['n_exit']:>6}{r['n_rec']:>6}")
    B1_ON, B1_PCTL, VOLTGT_ON, KILLSWITCH, SIGNAL_LAG = sv
    print("\n" + "=" * 104)
    print("  판독:")
    print("   · 지정된 PCTL 구간에서 대피 횟수가 전부 같다  → 임계 사망. 백분위는 아무 일도 안 함.")
    print("   · PCTL 올릴수록 대피 횟수가 줄어든다          → 임계 생존. 백분위가 필터링 중.")
    print("   · KS only 대피 0회 → 버블≥1.30 원조건은 무발동(무매와 동일). 대피는 전부 B1 소관.")
    print("=" * 104)


# ══════════════ [11. 진단 B-0b — 대피 '날짜'와 백분위 실측 ] ══════════════
def b1_signal_trace(df, start="2010-02-11"):
    global B1_ON, B1_PCTL, SIGNAL_LAG
    sv = (B1_ON, B1_PCTL, SIGNAL_LAG)
    B1_ON, B1_PCTL, SIGNAL_LAG = "on", 0.80, 1
    sub = df[df.index >= start]
    if END_DATE: sub = sub[sub.index <= END_DATE]
    sig = _signals(sub)
    print("\n" + "=" * 96)
    print(f"  🔬 [진단 B-0b] 대피 신호 발생일 추적 (시작 {start} · PCTL=0.80)")
    print("     '그날의 백분위'가 전부 0.95+ 라면 임계 0.80은 무의미(장식)")
    print("=" * 96)
    print(f"{'날짜':<13}{'버블':>8}{'백분위':>9}{'GSPC':>10}{'SMA200':>10}{'발동사유':>16}")
    print("-" * 96)
    prev_exit = False
    pctl_at_exit = []
    for dd in sub.index:
        ...
        if ex and not prev_exit:
            bub = sig["BU"].get(dd, np.nan)
            pc  = sig["PCTL"].get(dd, np.nan)
            g   = sig["G"].get(dd, np.nan)
            gs  = sig["GS"].get(dd, np.nan)
            why = "버블≥1.30" if (not pd.isna(bub) and bub >= BUBBLE_LIMIT) else "B1(백분위)"
            pctl_at_exit.append(pc)
            print(f"{str(dd.date()):<13}{bub:>8.2f}{pc:>9.3f}{g:>10.0f}{gs:>10.0f}{why:>16}")
        prev_exit = ex
    print("-" * 96)
    if pctl_at_exit:
        arr = pd.Series(pctl_at_exit).dropna()
        print(f"  대피 진입 {len(pctl_at_exit)}회 · 그날 백분위: "
              f"최소 {arr.min():.3f} / 중앙 {arr.median():.3f} / 최대 {arr.max():.3f}")
        below = (arr < 0.95).sum()
        print(f"  백분위 < 0.95 였던 대피: {below}회 / {len(arr)}회")
        if below == 0:
            print("  ⚠ 전부 0.95 이상 → 임계 0.80은 아무것도 거르지 않음(장식 확정)")
        else:
            print(f"  ✅ {below}회는 0.95 미만 → 임계가 실제로 구간을 가른다")
    pc_all = sub["BUB_PCTL"].dropna()
    print(f"\n  [참고] 전 기간 백분위 분포: "
          f"P10 {pc_all.quantile(.10):.2f} / 중앙 {pc_all.median():.2f} / P90 {pc_all.quantile(.90):.2f}")
    print(f"         백분위 ≥ 0.80 인 날의 비율: {(pc_all >= 0.80).mean()*100:.1f}%")
    print(f"         (이 비율이 높으면 → 임계 0.80이 상시 통과 = 장식)")
    B1_ON, B1_PCTL, SIGNAL_LAG = sv
    print("=" * 96)


# ══════════════ [12. 진단 E-1 — M0 소스 민감도] ══════════════
def m0_source_sensitivity(df_fred, df_fb, init=None):
    global B1_PCTL, B1_ON, VOLTGT_ON, SIGNAL_LAG, KILLSWITCH
    init = init or HOLD_CAP
    sv = (B1_PCTL, B1_ON, VOLTGT_ON, SIGNAL_LAG, KILLSWITCH)
    SIGNAL_LAG, VOLTGT_ON, KILLSWITCH, B1_ON = 1, "on", "on", "on"
    print("\n" + "=" * 100)
    print("  🔬 [진단 E-1] M0 소스 민감도 — B1이 데이터 소스에 종속되는가")
    print("     실 FRED vs 폴백앵커.  2022 대피가 양쪽에서 살아남으면 견고.")
    print("=" * 100)
    for label, d in [("실 FRED M0", df_fred), ("폴백 앵커보간", df_fb)]:
        print(f"\n▸ {label}   (버블 최신 {d['BUB'].iloc[-1]:.3f} · "
              f"2010~ 버블최대 {d['BUB'][d.index >= '2010-01-01'].max():.3f})")
        print(f"{'PCTL':<7}{'세후NAV':>15}{'CAGR':>8}{'MDD':>9}{'샤프':>7}{'대피':>6}")
        print("-" * 54)
        for p in (0.75, 0.80):
            B1_PCTL = p
            sub = d[d.index >= "2010-02-11"]
            if END_DATE: sub = sub[sub.index <= END_DATE]
            r = run_vr(sub, init, HOLD_POOL, HOLD_G, HOLD_LIMIT, killswitch=True)
            print(f"{p:<7.2f}{r['aftertax']:>15,.0f}{r['cagr']*100:>7.1f}%"
                  f"{r['mdd']*100:>8.1f}%{r['sharpe']:>7.2f}{r['n_exit']:>6}")
        ...
        rows = []
        for dd in sub.index:
            if not (pd.Timestamp("2022-01-01") <= dd <= pd.Timestamp("2022-12-31")):
                continue
            g = sig["G"].get(dd, np.nan); gs = sig["GS"].get(dd, np.nan)
            pc = sig["PCTL"].get(dd, np.nan)
            if pd.isna(g) or pd.isna(gs) or pd.isna(pc): continue
            if g < gs:
                rows.append((dd.date(), pc))
        if rows:
            pcs = pd.Series([p for _, p in rows])
            print(f"  2022 SMA200이탈일 {len(rows)}일 · 백분위 "
                  f"최소 {pcs.min():.3f} / 중앙 {pcs.median():.3f} / 최대 {pcs.max():.3f}")
            print(f"  → 0.75 넘김 {(pcs >= 0.75).sum()}일 / "
                  f"0.80 넘김 {(pcs >= 0.80).sum()}일  "
                  f"({'✅ 대피 발동' if (pcs >= 0.80).sum() > 0 else '❌ 2022 대피 소실!'})")
        else:
            print("  2022 SMA200 이탈일 없음")
    B1_PCTL, B1_ON, VOLTGT_ON, SIGNAL_LAG, KILLSWITCH = sv
    print("\n" + "=" * 100)
    print("  판독:")
    print("   · 두 소스 모두 2022 대피 발동 → B1 견고. 0.80도 안전.")
    print("   · 실 FRED만 발동, 폴백에서 소실 → 데이터 종속. 0.75로 여유 확보 필요.")
    print("   · 둘 다 소실 → B1 근본 재검토.")
    print("=" * 100)
