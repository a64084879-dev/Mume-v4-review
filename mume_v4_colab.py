...
# -*- coding: utf-8 -*-
"""
라오어 밸류 리밸런싱 백테스터 v2.3 — 거치식·적립식·인출식 (+킬스위치/B1/VOLTGT)  1986~오늘
════════════════════════════════════════════════════════════════════════
★ v2.3 (2026-07-31): 요약표·상세표에 'VR단독 MDD' 열 추가. 종전에는 VR+KS의 MDD만 찍혀
  '킬스위치를 끄면 낙폭이 얼마나 깊어지는가'를 같은 표에서 볼 수 없었다(§9 실행 중 확인 필요로 지적).
  이제 MDD_KS / MDD_단독 / ΔMDD 를 나란히 출력해 수익·낙폭 트레이드오프를 한눈에 대조한다.

★ v2.2 (2026-07-31): §9 B1 on/off 대조 추가. 배경 — B1의 원 채택 근거는 'VOLTGT와의
  초가산성'이었는데 VOLTGT가 off인 현재는 그 전제가 없다. 게다가 실데이터 16년의 대피는
  전부 B1발(버블 대피 0회)이고 KS효과가 5구간 중 4구간에서 1 미만이었다(v2.1 실행).
  ★판정 기준 사전 확정: 결론은 실데이터(2010~) 5구간으로만 내린다. 합성구간은 참고.
    한쪽이 실데이터 4구간 이상 일관 우세해야 인정. 동시에 대폭락 구간(1998·2000 시작)의
    MDD 악화폭을 함께 본다 — 최근 성과만 보고 끄면 그것이야말로 과최적화다.

★ v2.1 (2026-07-31): ① 매매수수료 FEE_RATE 전 매매 반영 — 토스 API 0.1% 실전 확정
    (매수 = 명목 b 지출 b×(1+f) · 매도 = 명목 s 수취 s×(1−f) · 0.0이면 v2 무수수료 재현).
  ② RECOVER_V_RESET 실험 옵션 — 복귀 시 V를 '복귀 직후 평가금'으로 리셋(on). 기본 off=책 규칙(V 유지).
  ③ VRESET_COMPARE — 거치식 hold를 off/on 두 번 돌려 §8 대조표 출력.
  ※ TAX_DEDUCTION=250.0 단위($250 vs 250만원≈$1,800) 확인 대기 — 이번 판 무변경.
...
FEE_RATE = 0.001                      # ★v2.1: 매매수수료. 토스 API 0.1% 실전 확정(2026-07-30). 0.0=v2 재현.
RECOVER_V_RESET = "off"               # ★v2.1 실험: 복귀 시 V 리셋. off=책 규칙(V 유지, 현행 봇과 동일).
VRESET_COMPARE  = "off"               # ★v2.2: §8은 판정 종료(V유지 확정, 186표본·10구간 전승) → 기본 off.
B1_COMPARE      = "on"                # ★v2.2: §9 대조표(거치식 hold, B1 on vs off). off=생략.
BUBBLE_LIMIT = 1.30
FAST_RECOVER = "on"
...
def run_vr(d, init_capital, pool_ratio, G, buy_limit, dep=0.0, wd=0.0, killswitch=True):
    ...
    shares = stock * (1.0 - FEE_RATE) / float(px.iloc[0]); V = shares * float(px.iloc[0])   # ★v2.1: 초기 매수 수수료
    cum_in = cum_out = 0.0
    ...
    for cd in split_cycles(px.index):
        ...
        while li < len(lumps) and lumps[li][0] <= cd[0] and state == "INVESTED":
            ...
            if total > 0 and V > 0:
                ...
                pool -= abs(amt) * w * FEE_RATE                     # ★v2.1: 목돈 매매분 수수료
                if pool < 0:
                    need = -pool; ss = min(need / p0, shares)
                    shares -= ss; pool += ss * p0 * (1.0 - FEE_RATE)   # ★v2.1
                    if pool < 0: pool = 0.0
        ...
        if state == "INVESTED":
            ...
            if pool < 0:                               # 인출로 현금 부족 → 주식 매도
                ...
                shares -= sell_sh; pool += sell_sh * p0 * (1.0 - FEE_RATE)   # ★v2.1
                if pool < 0: pool = 0.0

        # ★VOLTGT: 사이클 첫날에 '직전 거래일 RV'로 노출 확정 (봇의 cyc_scale 스냅샷)
        vsc = _vscale(sig, cd[0]); Veff = V * vsc               # ★v2.1: V리셋 시 재산출용으로 scale 보존
        ...
        for dd in cd:
            ...
            if killswitch:
                ...
                if state == "INVESTED":
                    ...
                    if _er:
                        pool += shares * p * (1.0 - FEE_RATE); shares = 0.0   # ★v2.1: 대피 매도 수수료
                        state = "CASH"; n_exit += 1; evac_reason = _er
                        if _er == "b1": n_exit_b1 += 1
                        else: n_exit_bubble += 1
                        if pool < min_nav: min_nav = pool; pool_at_min = pool   # ★최저NAV 시점(대피=전액현금)
                        daily.append((dd, pool)); continue
                # ★복귀: 전일(=월말) 신호 → 오늘 종가 집행. evac_reason으로 정책 적용.
                if state == "CASH":
                    _rw = _recover_which(sig, dd, evac_reason)
                    if _rw:
                        buy = min(Veff, pool / (1.0 + FEE_RATE))    # ★v2.1: 수수료 포함 지출 ≤ pool
                        shares = buy / p; pool -= buy * (1.0 + FEE_RATE)
                        state = "INVESTED"; n_rec += 1
                        if ON(RECOVER_V_RESET):                     # ★v2.1 실험: 복귀 시 V 리셋(off=책 규칙)
                            V = shares * p
                            Veff = V * vsc; bmin, bmax = Veff * BAND_LOW, Veff * BAND_HIGH
            ...
            if state == "INVESTED":
                ev = shares * p
                if ev < bmin:
                    b = min(bmin - ev, pool / (1.0 + FEE_RATE), max(0, budget - used))   # ★v2.1
                    if b > 1e-9:
                        shares += b / p; pool -= b * (1.0 + FEE_RATE); used += b; nb += 1
                elif ev > bmax:
                    s = ev - bmax
                    if s > 1e-9:
                        shares -= s / p; pool += s * (1.0 - FEE_RATE); ns += 1   # ★v2.1
...
def run_hold_bench(px, init_capital, dep=0.0, wd=0.0):
    ...
    shares = init_capital * (1.0 - FEE_RATE) / float(px.iloc[0]); cum_in = cum_out = 0.0   # ★v2.1
    for cd in split_cycles(px.index):
        p0 = float(px.loc[cd[0]])
        if dep > 0: shares += dep * (1.0 - FEE_RATE) / p0; cum_in += dep            # ★v2.1
        if wd > 0:  shares -= min(wd / ((1.0 - FEE_RATE) * p0), shares); cum_out += wd  # ★v2.1
...
def _table(df, title, init, pool_ratio, G, buy_limit, dep, wd):
    ...
    cols = [("시작일", 12, "<"), ("년수", 6, ">"), ("원금", 11, ">"), ("VR+KS세후", 15, ">"),
            ...
            ("CAGR", 8, ">"), ("MDD_KS", 9, ">"), ("MDD_단독", 10, ">"), ("ΔMDD", 8, ">"),
            ("샤프", 7, ">"), ("대피/복귀", 11, ">")]
    ...
    for sd in START_DATES:
        ...
        vals = [(sd, 12, "<"), (f"{rk['yrs']:.1f}", 6, ">"), (f"{rk['cum']:,.0f}", 11, ">"),
                ...
                (f"{rn['mdd']*100:.1f}%", 10, ">"),
                (f"{(rk['mdd']-rn['mdd'])*100:+.1f}%", 8, ">"),     # ★v2.3: ΔMDD(+면 KS가 얕게)
...
def _nav_series_vr(d, init, pool_ratio, G, buy_limit, killswitch, dep=0.0, wd=0.0):
    ...
    shares = stock * (1.0 - FEE_RATE) / float(px.iloc[0]); V = shares * float(px.iloc[0])   # ★v2.1
    daily = []; state = "INVESTED"; evac_reason = "bubble"; px_peak = 0.0
    ...
    for cd in split_cycles(px.index):
        ...
        for dd in cd:
            p = float(px.loc[dd])
            if p > px_peak: px_peak = p
            if killswitch:
                if state == "INVESTED":
                    _er = _exit_reason(sig, dd)
                    if _er:
                        pool += shares * p * (1.0 - FEE_RATE); shares = 0.0; state = "CASH"; evac_reason = _er   # ★v2.1
                        daily.append((dd, pool)); continue
                if state == "CASH" and _recover_sig(sig, dd, evac_reason):
                    buy = min(Veff, pool / (1.0 + FEE_RATE))        # ★v2.1
                    shares = buy / p; pool -= buy * (1.0 + FEE_RATE); state = "INVESTED"
                    if ON(RECOVER_V_RESET):
                        V = shares * p; Veff = V * vsc; bmin, bmax = Veff * BAND_LOW, Veff * BAND_HIGH
            if state == "INVESTED":
                ev = shares * p
                if ev < bmin:
                    b = min(bmin - ev, pool / (1.0 + FEE_RATE), max(0, budget - used))   # ★v2.1
                    if b > 1e-9:
                        shares += b / p; pool -= b * (1.0 + FEE_RATE); used += b
                elif ev > bmax:
                    s = ev - bmax
                    if s > 1e-9:
                        shares -= s / p; pool += s * (1.0 - FEE_RATE)   # ★v2.1
            daily.append((dd, shares * p + pool))

        if state == "INVESTED":
            E = shares * float(px.loc[cd[-1]])
            _pctl = d["BUB_PCTL"].get(cd[-1], np.nan) if "BUB_PCTL" in d.columns else np.nan
            _use_skill = _skill_on(E, V, float(px.loc[cd[-1]]), px_peak, _pctl)
            skill = (E - V) / (2 * np.sqrt(G)) if _use_skill else 0.0
            V = V + pool / G + skill + flow
...
def summary(df):
    ...
    print(f"{'시작일':<13}{'년수':>6}{'VR+KS':>14}{'VR단독':>14}{'TQQQ보유':>14}"
          f"{'CAGR':>8}{'MDD_KS':>9}{'MDD_단독':>10}{'ΔMDD':>8}{'샤프':>7}{'KS효과':>9}")
    print("-" * 110)
    for sd in REAL_STARTS:
        ...
        _dm = (rk["mdd"] - rn["mdd"]) * 100          # ★v2.3: +면 KS가 낙폭을 얕게 만듦
        print(f"{sd:<13}{rk['yrs']:>5.1f}년{rk['aftertax']:>14,.0f}{rn['aftertax']:>14,.0f}"
              f"{ht:>14,.0f}{rk['cagr']*100:>7.1f}%{rk['mdd']*100:>8.1f}%"
              f"{rn['mdd']*100:>9.1f}%{_dm:>+7.1f}%{rk['sharpe']:>7.2f}{eff:>8.2f}배")
    print("-" * 110)
    print("  · KS효과 = VR+킬스위치 ÷ VR단독(세후 자산 배수). 1 미만이면 킬스위치가 수익을 깎았다는 뜻.")
    print("  · ★MDD_KS / MDD_단독 / ΔMDD(+면 KS가 낙폭을 얕게) — 수익 배수만 보지 말고 반드시 함께 볼 것.")
...
# ══════════════ [8. 복귀 V리셋 실험 — 거치식 hold, 유지 vs 리셋] ══════════════
def vreset_compare(df):
    """RECOVER_V_RESET off(책 규칙=V 유지) vs on(복귀 직후 평가금으로 리셋)을
       거치식 hold로 전 시작일 대조. ★판정 기준(사전 확정): 대피 표본 합계 10회 이상이고
       8구간 중 6구간 이상 일관 우세일 때만 의미 있는 차이로 본다.
       특정 1~2 구간만 좋으면 기각(과최적화 규칙). 2010 이전 시작은 합성 데이터 → 참고용."""
    global RECOVER_V_RESET
    keep = RECOVER_V_RESET
    print("\n" + "█" * 100)
    print("  ★ §8. 복귀 V리셋 실험 — 거치식 {:,.0f} · 세후 · FEE {:.2%} · KS={} B1={}".format(
        HOLD_CAP, FEE_RATE, KILLSWITCH, B1_ON))
    print("█" * 100)
    hdr = (f"{'시작일':<12}{'대피(버블/B1)':>14}{'복귀':>6}"
           f"{'CAGR유지':>10}{'CAGR리셋':>10}{'Δpp':>8}"
           f"{'MDD유지':>9}{'MDD리셋':>9}{'ΔMDD':>8}  판정")
    print(hdr); print("─" * len(hdr.expandtabs()))
    tot_exit = 0; win_keep = win_reset = 0
    for sd in START_DATES:
        sub = df[df.index >= sd]
        if END_DATE: sub = sub[sub.index <= END_DATE]
        if len(sub) < 300: continue
        RECOVER_V_RESET = "off"
        a = run_vr(sub, HOLD_CAP, HOLD_POOL, HOLD_G, HOLD_LIMIT, killswitch=ON(KILLSWITCH))
        RECOVER_V_RESET = "on"
        b = run_vr(sub, HOLD_CAP, HOLD_POOL, HOLD_G, HOLD_LIMIT, killswitch=ON(KILLSWITCH))
        RECOVER_V_RESET = keep
        tot_exit += a["n_exit"]
        d_c = (b["cagr"] - a["cagr"]) * 100
        d_m = (b["mdd"] - a["mdd"]) * 100
        tag = "참고(합성)" if sd < "2010-02-11" else "실데이터"
        if abs(d_c) < 0.05: verdict = "동률"
        elif d_c > 0: verdict = "리셋 우세"; win_reset += 1
        else: verdict = "유지 우세"; win_keep += 1
        print(f"{sd:<12}{a['n_exit_bubble']:>7}/{a['n_exit_b1']:<6}{a['n_rec']:>6}"
              f"{a['cagr']*100:>9.2f}%{b['cagr']*100:>9.2f}%{d_c:>+8.2f}"
              f"{a['mdd']*100:>8.1f}%{b['mdd']*100:>8.1f}%{d_m:>+7.1f}%  {verdict} · {tag}")
    print("─" * 100)
    print(f"  · 대피 표본 합계 {tot_exit}회 | 유지 우세 {win_keep}구간 · 리셋 우세 {win_reset}구간 (Δ<0.05pp=동률)")
    print("  · 채택 조건: 표본 ≥10 AND 한쪽이 6구간 이상 일관 우세. 미달이면 현행(V 유지) 유지.")
    print("█" * 100)


# ══════════════ [9. B1 on/off 대조 — 확장 대피장치의 단독 가치 재검증] ══════════════
def b1_compare(df):
    """B1 = 버블<1.30이어도 롤링백분위≥B1_PCTL AND GSPC<SMA200 이면 대피(확장).
       대폭락 방어는 기본 킬스위치(버블≥1.30)가 담당하며 B1과 무관하다.
       ★판정(사전 확정): 결론은 실데이터 5구간(2010~2022)으로만. 합성 4구간은 참고.
         한쪽이 실데이터 4구간 이상 일관 우세해야 인정. 대폭락 시작(1998·2000)의
         MDD 악화폭을 반드시 함께 볼 것 — 최근 성과만 보고 끄면 과최적화."""
    global B1_ON
    keep = B1_ON
    print("\n" + "█" * 104)
    print("  ★ §9. B1 on/off 대조 — 거치식 {:,.0f} · 세후 · FEE {:.2%} · KS={} VOLTGT={} PCTL={}".format(
        HOLD_CAP, FEE_RATE, KILLSWITCH, VOLTGT_ON, B1_PCTL))
    print("█" * 104)
    hdr = (f"{'시작일':<12}{'대피 on(버블/B1)':>16}{'대피 off':>9}"
           f"{'CAGR_on':>9}{'CAGR_off':>10}{'Δpp':>8}"
           f"{'MDD_on':>9}{'MDD_off':>9}{'ΔMDD':>8}  판정")
    print(hdr); print("─" * 104)
    real_on = real_off = 0; syn_on = syn_off = 0
    for sd in START_DATES:
        sub = df[df.index >= sd]
        if END_DATE: sub = sub[sub.index <= END_DATE]
        if len(sub) < 300: continue
        ...
        b = run_vr(sub, HOLD_CAP, HOLD_POOL, HOLD_G, HOLD_LIMIT, killswitch=ON(KILLSWITCH))
        B1_ON = keep
        d_c = (b["cagr"] - a["cagr"]) * 100          # +면 B1 off가 유리
        d_m = (b["mdd"] - a["mdd"]) * 100            # +면 off의 MDD가 얕음(유리)
        real = sd >= "2010-02-11"
        if abs(d_c) < 0.05: verdict = "동률"
        elif d_c > 0:
            verdict = "B1 off 우세"
        ...
        print(f"{sd:<12}{a['n_exit_bubble']:>8}/{a['n_exit_b1']:<7}{b['n_exit']:>9}"
              f"{a['cagr']*100:>8.2f}%{b['cagr']*100:>9.2f}%{d_c:>+8.2f}"
              f"{a['mdd']*100:>8.1f}%{b['mdd']*100:>8.1f}%{d_m:>+7.1f}%  {verdict}"
              f" · {'실데이터' if real else '참고(합성)'}")
    print("─" * 104)
    print(f"  · [실데이터 5구간] B1 on 우세 {real_on} · off 우세 {real_off}")
    print(f"  · [합성 4구간·참고] B1 on 우세 {syn_on} · off 우세 {syn_off}")
    print("  · 판정: 실데이터에서 한쪽이 4구간 이상 일관 우세해야 변경 검토. 미달이면 현행(B1=on) 유지.")
    print("  · ★함께 볼 것: 1998·2000 시작(닷컴·금융위기 포함)의 ΔMDD. off가 크게 나빠지면")
    print("    최근 구간 CAGR 우위만으로 끄는 것은 위험하다(대폭락 방어는 기본 킬스위치 몫이나,")
    print("    B1이 실제로 얕은 조정에서 추가 방어를 했는지 여기서 드러난다).")
    print("█" * 104)
...
if __name__ == "__main__":
    ...
    print(f"  · FEE_RATE={FEE_RATE:.3%} (토스 API 실전 확정 · 0이면 v2 무수수료 재현) | RECOVER_V_RESET={RECOVER_V_RESET}")
    print(f"  · §8 VRESET_COMPARE={VRESET_COMPARE}(판정종료:V유지) | §9 B1_COMPARE={B1_COMPARE}")
