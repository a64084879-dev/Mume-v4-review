...
#         A′ = FAST 48:52 단독(눈금 1.44) — v3 추가. 눈금을 B와 통일해 '냉각재 재질' 효과만 분리하는
#              통제 실험. 이 한 팔로 확장 종료 — 추가 비중(46, 44 등) 영구 금지, 스윕 봉인 불변.
...
W_AP = {'TQQQ': 0.48, 'gold': 0.52}  # ★v3 R5 A′팔: 눈금 1.44 통제 실험(추가 비중 영구 금지)
BUBBLE_LIMIT = 1.30
...
ARMS = [("A", "FAST 50:50 단독(눈금 1.50)", None, 0.0),
        ("A'", "FAST 48:52 단독(눈금 1.44)", None, 0.0),
        ("B", "FAST 60:40 80% + 방석 20%(눈금 1.44)", None, B_CUSH_W),
        ("C", "FAST 60:40 단독(기준선)", None, 0.0)]

def _arm_weights(tag):
    if tag == "A": return W_B                  # 정본 99행 W_B 원문
    if tag == "A'": return W_AP                # ★v3 A′: 48:52(눈금 1.44)
    return W_A                                 # B·C = W_A
...
def main():
    ...
    print("  · 각주(필수·v3): A′는 눈금 통제 실험(1.44 동일) — 냉각재 재질(금 52% 대 방석 20%)과")
    print("    세금 서랍(계좌 내 과세 대 KRX 비과세)의 효과 분리 목적. 관찰 전용·봉인 불변·추가 비중 영구 금지.")
    ...
    # ── V9 정합(대표 3창) ──
    print("\n" + "-" * 100)
    print(f"  [V9] 정합 — (a) C(이식본) ↔ 정본(W_A) / (b) A(이식본) ↔ 정본(W_B) / (c) A′(이식본) ↔ 정본(48:52)")
    ...
    for mode, y, win in [w for w in wins if w[0] == "R1" and w[1] in (1994, 2000, 2005)]:
        ...
        rap = run_arm(D, win, W_AP, 0.0)                    # ★v3 A′ = 이식본 FAST 48:52 단독
        nav_ap, _ = run_simulation(D.loc[win], R5_INIT, W_AP, method='fast_recover')
        dc = (cagr(float(rap['nav'].iloc[-1])) - cagr(float(nav_ap.iloc[-1]))) * 100
        okA, okB, okC = abs(da) <= V9_TOL, abs(db) <= V9_TOL, abs(dc) <= V9_TOL
        print(f"    [{y}] (a) C(이식본) vs 정본(W_A): {cagr(float(rc['nav'].iloc[-1]))*100:6.2f}% vs "
              ...
              f"(c) A′ vs 정본(48:52): {cagr(float(rap['nav'].iloc[-1]))*100:6.2f}% vs "
              f"{cagr(float(nav_ap.iloc[-1]))*100:6.2f}% → 차 {dc:+.3f}%p {'PASS' if okC else '★FAIL'}")
        if abs(da) > V9_REPORT or abs(db) > V9_REPORT or abs(dc) > V9_REPORT:
            print(f"        · 보고: 오차 {V9_REPORT}%p 초과 — 원인 = 만기 청산·납부 경로의 통합화(R4 V8 기실측 동일 사유)")
        assert okA and okB and okC, f"V9 실패: {y}"
    ...
    try:
        ...
        _setup_korean_font()
        colors = {"A": "#1f77b4", "A'": "#2ca02c", "B": "crimson", "C": "#7f7f7f"}
