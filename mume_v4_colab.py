# -*- coding: utf-8 -*-
# ============================================================================
# kelly17_research_r3.py — 연구사양서 R3 방석재질 v1.1(md5 baaa1510) 구현 (2026-08-14, Gemini 역할)
...
def main():
    ...
    for mode, y, win in wins:
        ...
        for name, rA, rB, wA, wB in mats:
            ...
            r_csh = cush / np.r_[CUSH_W, cush[:-1]] - 1.0
            mdd = float((np.r_[1.0, pouch] / np.maximum.accumulate(np.r_[1.0, pouch]) - 1.0).min())
            vol = float(r_pch.std(ddof=0) * np.sqrt(252.0))
            if name == "단기채":
                vol_bond = vol
            cmean = float(r_csh[mask].mean() * 100) if ncr else float('nan')
            cworst = float(r_csh[mask].min() * 100) if ncr else float('nan')
            ...
            if ncr: a["crisis"].append(r_csh[mask])
            if mode == "R1":
