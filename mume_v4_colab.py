...
def print_voltgt(df):
    ...
    print(_pad("방식",8)+_pad("최종NAV",14,">")+_pad("CAGR",8,">")+_pad("MDD장중",10,">")+_pad("샤프",7,">")+_pad("사이클",8,">")+_pad("리버스",7,">")+_pad("투자%",7,">"))
...
def print_voltgt_multi(df_full, start_dates):
    ...
    print(_pad("시작일",12)+_pad("PURE CAGR",11,">")+_pad("PURE MDD",10,">")+" |"+_pad("A CAGR",9,">")+_pad("A MDD",9,">")+_pad("AΔMDD",8,">")+_pad("A샤프",7,">")+" |"+_pad("B CAGR",9,">")+_pad("B MDD",9,">")+_pad("BΔMDD",8,">")+_pad("B샤프",7,">"))
...
def print_killswitch(df):
    ...
    print(_pad("모드",10)+_pad("최종NAV",14,">")+_pad("CAGR",8,">")+_pad("MDD",9,">")+_pad("샤프",7,">")+_pad("사이클",8,">")+_pad("대피일",8,">")+_pad("투자%",7,">"))
...
​
    ...
    if START_DATES:
        ...
        print(_pad("시작일",12)+_pad("PURE CAGR",10,">")+_pad("PURE MDD",10,">")+_pad("PURE샤프",9,">")+" |"+_pad("B1 CAGR",9,">")+_pad("B1 MDD",9,">")+_pad("B1 ΔMDD",9,">")+_pad("B1샤프",8,">")+_pad("대피일",8,">"))
...
def print_voltgt_regime(df, sig=None):

    """은박사 가설 검증: VOLTGT가 평시엔 수익만 깎고 위기에만 도움 되는가?

    두 기준으로 분해:

      ① scale 기준: VOLTGT가 실제 '작동한 날'(scale<1) vs '안 한 날'(scale=1)

         → VOLTGT가 개입한 날 성과가 나쁘면 '수익 갉아먹음' 확증

      ② 드로다운 기준: 고점 대비 -30%↓(위기) vs 나머지(평시)"""

    print("\n"+"="*100)

    print("  VOLTGT 평시/위기 분해 — '평시엔 수익만 깎는가' 검증")
    ...
    scale = np.minimum(1.0, VOLTGT_TARGET / rv)

    peak = px.cummax(); dd = px/peak - 1.0

    crisis = dd <= -0.30                    # 고점 -30%↓ = 위기

    acted  = (scale < 0.999) & rv.notna()   # VOLTGT가 실제 개입한 날

​

    print(f"  VOLTGT 개입일 {int(acted.sum())}일 / 미개입 {int((~acted & rv.notna()).sum())}일")

    print(f"  위기(고점-30%↓) {int(crisis.sum())}일 / 평시 {int((~crisis).sum())}일")

    print(f"  · 평시 중 VOLTGT 개입: {int((acted & ~crisis).sum())}일  ← 이게 많으면 '평시 간섭'")

    print(f"  · 위기 중 VOLTGT 개입: {int((acted & crisis).sum())}일")

    print("-"*100)

​

    rp = run_voltgt(df, mode="PURE"); ra = run_voltgt(df, mode="A")

    navp = pd.Series(rp.nav, index=rp.dates); nava = pd.Series(ra.nav, index=ra.dates)

    retp = navp.pct_change().fillna(0); reta = nava.pct_change().fillna(0)
...
​
    ...
    def cum(s, mask):
        ...
        return ((1+s[m]).prod()-1)*100

​

    print(f"{_pad('구간',22)}{_pad('일수',8,'>')}{_pad('PURE 누적',12,'>')}{_pad('A 누적',12,'>')}{_pad('차이(A-P)',12,'>')}")

    print("-"*100)

    segs = [("평시 전체", ~crisis),

            ("  └ 평시·VOLTGT개입", acted & ~crisis),

            ("위기 전체(-30%↓)", crisis),

            ("  └ 위기·VOLTGT개입", acted & crisis)]

    for label, mask in segs:

        cp = cum(retp, mask); ca = cum(reta, mask)

        n = int(mask.reindex(navp.index).fillna(False).sum())

        print(f"{_pad(label,22)}{_pad(str(n),8,'>')}{_pad(f'{cp:+.1f}%',12,'>')}{_pad(f'{ca:+.1f}%',12,'>')}{_pad(f'{ca-cp:+.1f}%p',12,'>')}")

    print("="*100)

    print("  판독: '평시·VOLTGT개입' 차이가 음수 = 평시에 수익 깎음(은박사 가설).")

    print("        '위기·VOLTGT개입' 차이가 양수 = 위기엔 도움. 둘의 크기 비교가 핵심.")

    print("="*100)

​

def print_voltgt_sweep(df, targets=(0.50,0.60,0.70,0.80,0.90,1.00,1.20)):

    """VOLTGT 타겟 스윕 — 과적합 판정용.

    ⚠ 특정 타겟만 좋으면 과적합. 넓은 구간서 평탄하게 좋아야 견고(VR B1 판정 기준과 동일).

    각 타겟에서 평시개입일·CAGR·MDD·샤프를 보고, '평시 간섭 없이 위기만 잡는' 지점을 찾음."""

    global VOLTGT_TARGET

    orig = VOLTGT_TARGET

    px = df["Close"]; rv = rv_series(px)
...
​
    ...
    print("  VOLTGT 타겟 스윕 — 과적합 검증 (평탄하면 견고, 한 점만 좋으면 과적합)")

    print("="*104)

    rp = run_voltgt(df, mode="PURE")

    pc, pm, ps = metrics(rp.nav, rp.dates, SEED, rp.nav_low)

    print(f"  기준(PURE): CAGR {pc*100:.1f}% · MDD {pm*100:.1f}% · 샤프 {ps:.2f}")

    print("-"*104)

    print(_pad("타겟",8)+_pad("개입일",8,">")+_pad("평시개입",9,">")+_pad("위기개입",9,">")

          +_pad("CAGR",8,">")+_pad("ΔCAGR",8,">")+_pad("MDD",9,">")+_pad("ΔMDD",8,">")

          +_pad("샤프",7,">")+_pad("Δ샤프",8,">"))
    ...
    for tg in targets:
        ...
        sc = np.minimum(1.0, tg/rv)

        acted = (sc < 0.999) & rv.notna()

        n_all = int(acted.sum())

        n_calm = int((acted & ~crisis).sum())

        n_cri  = int((acted & crisis).sum())

        r = run_voltgt(df, mode="A")

        cg, md, sh = metrics(r.nav, r.dates, SEED, r.nav_low)

        print(_pad(f"{tg*100:.0f}%",8)+_pad(str(n_all),8,">")+_pad(str(n_calm),9,">")+_pad(str(n_cri),9,">")

              +_pad(f"{cg*100:.1f}%",8,">")+_pad(f"{(cg-pc)*100:+.1f}",8,">")

              +_pad(f"{md*100:.1f}%",9,">")+_pad(f"{(md-pm)*100:+.1f}",8,">")

              +_pad(f"{sh:.2f}",7,">")+_pad(f"{sh-ps:+.2f}",8,">"))

    VOLTGT_TARGET = orig

    print("="*104)

    print("  판독: 타겟↑ → 개입 줄어듦(평시 간섭↓). Δ샤프가 넓은 구간서 +면 견고, 한 점만 +면 과적합.")

    print("        평시개입이 0에 가까우면서 Δ샤프 +면 '위기에만 작동' = 은박사 가설의 이상적 지점.")
...
if RUN_VOLTGT:
    ...
    print_voltgt_regime(df)          # 은박사 가설: 평시엔 수익만 깎는가?

    print_voltgt_sweep(df)           # 타겟 상향 실험 (과적합 판정: 평탄해야 견고)
