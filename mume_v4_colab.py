...
EXIT_INDEX = "GSPC"   # "GSPC" | "NDX" — 탈출 추세 판정 지수
...
if __name__ == "__main__":
    ...
    print(f"  · ★F7/F8/F9: NAV=자산−확정세부채(잠재세 폐지, VR v3 정합) · 수수료 편도: 미국주식 "
          f"{(NORMAL_SLIPPAGE+COMMISSION)*100:.2f}%(토스 실측) · gold 0.30% · BOXX 0.15% · MDD=청산 전 경로")
    ...
    if START_DATES:
        ...
        for _sd_i in START_DATES:
            ...
            if len(_sub) < 250:
                ...
                continue
            for _lbl, _w in [(f"A({next((k for k in W_A if k != 'TQQQ'), '')}{int(round(W_A.get('TQQQ', 0) * 100))})", W_A),
                             (f"B({next((k for k in W_B if k != 'TQQQ'), '')}{int(round(W_B.get('TQQQ', 0) * 100))})", W_B)]:
            ...
            print(f"{_pad(_sd_i,12)} | {_pad('TQQQ단독(세후)',14)} | {_nav_t.iloc[-1]:>16,.0f} | "
            ...
            print(f"{_pad(_sd_i,12)} | {_pad('QLD단독(세후)',14)} | {_nav_q.iloc[-1]:>16,.0f} | "
            ...
            print(f"{_pad(_sd_i,12)} | {_pad('QQQ단독(세후)',14)} | {_nav_n.iloc[-1]:>16,.0f} | "
            ...
            print(f"{_pad(_sd_i,12)} | {_pad('SPY단독(세후)',14)} | {_nav_s.iloc[-1]:>16,.0f} | "
