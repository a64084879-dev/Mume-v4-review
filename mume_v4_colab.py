...
def run():
    ...
    if not DRY_RUN:
        ...
        if _oid:
            ...
            def _oid_of(k):
                _p=str(k).split(":")
                return {_p[-2] if len(_p)>=3 else None, _p[-1] if len(_p)>=2 else None}
            _filled = any(str(_oid) in _oid_of(k) for k in (pos.get("fills_seen") or {}))
            if _filled:
    ...
    head += bot.render_echo_head(pos, cmd_results)   # ★R-A(2026-07-24): 통지 재전송 안전망을 러너에도 미러.
                                                     #   누락 시 키움 경로에서 ⏮가 안 뜨고 echo가 최대 11개까지 누적됐다.
    if cmd_results:
