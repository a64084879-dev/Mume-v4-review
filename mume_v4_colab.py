...
def _signals(d):
    ...
    sig = {
        "G":    sh(d["GSPC"]),   # ★R6-b 실험판: 탈출 기준선 = S&P500 (2026-08-16 은박사님 재가)
        "GS":   sh(d["GSMA"]),
...
def run_r4(D, win, x_pct, m_monthly, fast_init=FAST_INIT, vr_init=VR_INIT,
           vr_stock_w=VR_START_STOCK_W, narrate=False):
    ...
    for i, cd in enumerate(dates):
        ...
        # ④ FAST 신호(정본: GATE=ABS·EXIT=EXIT_INDEX·복귀 GSPC 월말+fast_recover)
...
def main_sweep(D, self_md5):
    ...
    S = pd.DataFrame(rows)
    S.to_csv("summary_r6b_sweep.csv", index=False, encoding="utf-8-sig")
    ...
    print(f"      summary_r6b_sweep.csv : {_md5_file('summary_r6b_sweep.csv')}  ({len(S)}행)")
    ...
    try:
        ...
        plt.tight_layout(); plt.savefig("r6b_sweep_summary.png", dpi=100, bbox_inches="tight")
        print("  · 차트 저장: r6b_sweep_summary.png")
        if in_nb:
            try: plt.show()
        ...
        for ykey, store in sorted(navs.items()):
            ...
            plt.tight_layout(); out = f"r6b_chart_{ykey}.png"
