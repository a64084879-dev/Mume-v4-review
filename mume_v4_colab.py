...
VAR_LAB          = False        # True일 때만 [5v] 섹션 실행
...
if __name__ == "__main__":
    ...
    if START_DATES:
        ...
        print("=" * 108)
        print(f"{_pad('시작일',12)} | {_pad('자산',18)} | {_pad('최종자산($)',16)} | {_pad('CAGR',8)} | {_pad('MDD',8)} | {_pad('샤프',6)} | {_pad('대피',5)}")
        print("-" * 108)
        for _sd_i in START_DATES:
            ...
            for _lbl, _w in [(A_NAME, W_A), (B_NAME, W_B)]:
                ...
                print(f"{_pad(_sd_i,12)} | {_pad(_lbl,18)} | {_nav_i.iloc[-1]:>16,.0f} | "
                      f"{_c_i*100:>7.2f}% | {_m_i*100:>7.2f}% | {_s_i:>6.2f} | {_nd_i:>5}")
            _nav_t = run_bh_aftertax(_sub, INITIAL_CAPITAL, 'TQQQ')           # ★F12: TQQQ 단독(세후)
            _c_t, _m_t, _s_t = calc_stats(_nav_t, INITIAL_CAPITAL)
            print(f"{_pad(_sd_i,12)} | {_pad('TQQQ단독(세후)',18)} | {_nav_t.iloc[-1]:>16,.0f} | "
                  f"{_c_t*100:>7.2f}% | {_m_t*100:>7.2f}% | {_s_t:>6.2f} | {'-':>5}")
            _nav_q = run_bh_aftertax(_sub, INITIAL_CAPITAL, 'QLD')            # ★F13: QLD 단독(세후)
            _c_q, _m_q, _s_q = calc_stats(_nav_q, INITIAL_CAPITAL)
            print(f"{_pad(_sd_i,12)} | {_pad('QLD단독(세후)',18)} | {_nav_q.iloc[-1]:>16,.0f} | "
                  f"{_c_q*100:>7.2f}% | {_m_q*100:>7.2f}% | {_s_q:>6.2f} | {'-':>5}")
            _nav_n = run_bh_aftertax(_sub, INITIAL_CAPITAL, 'QQQ')            # ★F14: QQQ 단독(세후)
            _c_n, _m_n, _s_n = calc_stats(_nav_n, INITIAL_CAPITAL)
            print(f"{_pad(_sd_i,12)} | {_pad('QQQ단독(세후)',18)} | {_nav_n.iloc[-1]:>16,.0f} | "
                  f"{_c_n*100:>7.2f}% | {_m_n*100:>7.2f}% | {_s_n:>6.2f} | {'-':>5}")
            _nav_s = run_bh_aftertax(_sub, INITIAL_CAPITAL, 'SPY')            # ★F14: SPY 단독(세후)
            _c_s, _m_s, _s_s = calc_stats(_nav_s, INITIAL_CAPITAL)
            print(f"{_pad(_sd_i,12)} | {_pad('SPY단독(세후)',18)} | {_nav_s.iloc[-1]:>16,.0f} | "
                  f"{_c_s*100:>7.2f}% | {_m_s*100:>7.2f}% | {_s_s:>6.2f} | {'-':>5}")
            print("-" * 108)
        print("=" * 108)
        _bh = next((k for k in RECOVER_BOOST if k != 'TQQQ'), '')
        print(f"  · 부스터B(NDX 단독복귀 시 TQQQ{int(round(RECOVER_BOOST['TQQQ']*100))}:{_bh}{int(round(RECOVER_BOOST.get(_bh,0)*100))}) 기준. "
              f"라벨은 W_A·W_B 구성에서 자동 생성(자산 변경 시 자동 반영).")
